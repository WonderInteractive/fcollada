...
xmlFdRead (void * context, char * buffer, size_t len) {
    ...
    ret = _read((int) (size_t) context, &buffer[0], (unsigned int) len);
...
xmlFdWrite (void * context, const char * buffer, size_t len) {
    ...
    ret = _write((int) (size_t) context, &buffer[0], (unsigned int) len);
...
#ifdef __PPU__
        // getcwd is not directly supported on PS3
	    ret = xmlMemStrdup(".");
#else
